...
Independent verification remains required for findings that lack multi-reviewer corroboration.

1. Skip a validator only when the finding has `first_evidence` and is independently corroborated by two distinct persona reviewers. Record the skip count in Coverage.
...
**Report completion gate:** do not finish until stable `#` identifiers appear on every primary finding and the report contains `### Actionable Findings`, `### Coverage`, and `### Verdict` (or their exact JSON fields in `mode:agent`). Coverage must name the reviewer outcome and validator shortcut/batch outcome. The Actionable section must include every `downstream-resolver` finding; never silently replace it with a count.
