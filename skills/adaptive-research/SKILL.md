...
1. **Goal Discovery (Research Formulation):** Deconstruct the topic into clear, falsifiable hypotheses and establish specific orthogonal evaluation dimensions (quantitative and qualitative). 
   - **Filesystem Init:** You MUST create a root directory for this task in a strictly isolated location OUTSIDE the user's codebase (e.g., `/tmp/<Research_Topic_Snake_Case>_Adaptive_Research/` or `<appDataDir>/brain/<conversation-id>/scratch/`). **NEVER** create this directory in the current working directory. Save your research plan, hypotheses, and persona definitions to `<root>/orchestrator_plan.md`.
   - Do not ask the user for clarification; if underspecified, default to an exhaustive multi-source comparative analysis. Provide a brief status update outlining the research plan and generated personas before dispatching.
...
4. **Dispatch:** You MUST use the `view_file` tool to read `references/focused-research.md` and `references/findings_schema.md`.
   - **Filesystem Setup:** You MUST physically create a `personas/<subagent_name>/` directory inside your `<root>` folder for each persona BEFORE invoking them.
   - Dispatch the generated personas concurrently using `invoke_subagent`.
   - You must pass the *absolute file paths* of `references/focused-research.md`, `references/findings_schema.md`, AND the absolute path to their dedicated `personas/<subagent_name>/` workspace directly into the subagents' `Prompt`. Instruct them to use `view_file` to read their protocol and schema before starting.
...
5. **Synthesis (Anti-Bias & Triangulation):** After returns are ready, parse the structured data.
   - Read the `findings.json` from each persona's workspace directory. If a subagent timed out or returned malformed JSON violating the schema, discard the invalid data and proceed with successful subagents.
...
6. **Conclusiveness Check (Visual Density):** Deliver the final synthesized report to the user by writing it to `<root>/synthesis/adaptive_research_synthesis_final.md`.
   ...
   - Use syntax-verified **Mermaid diagrams** (e.g., flowcharts, sequence diagrams) to visually contrast competing architectural theories or workflows. Save any standalone diagrams to `<root>/synthesis/diagrams/`.
...
| `references/findings_schema.md` | Stage 4 | The strict JSON schema subagents must use for their final report |
